...
expected =
 ...
 ExprCase {name = "602"; test = Semantic_Analysis.run_semantics;
  input =
        ScmDef (ScmVar "test",
        ScmLambdaOpt (["x"], "y",
          ScmApplic (ScmVar "cons", [
              ScmVar "x";
              ScmLambdaSimple ([], ScmSet (ScmVar "x", ScmVar "c"));
              ScmLambdaSimple ([], ScmSeq ([
                  ScmVar "y";
                  ScmSet (ScmVar "y", ScmVar "a");
                  ScmApplic (ScmVar "b", [])]))])));
  expected = ScmDef' (VarFree "test",
  ScmLambdaOpt' (["x"], "y",
      ScmSeq' ([
          ScmSet' (VarParam ("x", 0), ScmBox' (VarParam ("x", 0)));
          ScmApplicTP' (ScmVar' (VarFree "cons"), [
              ScmBoxGet' (VarParam ("x", 0));
              ScmLambdaSimple' ([], ScmBoxSet' (VarBound ("x", 0, 0), ScmVar' (VarFree "c")));
              ScmLambdaSimple' ([], ScmSeq' ([
                  ScmVar' (VarBound ("y", 0, 1));
                  ScmSet' (VarBound ("y", 0, 1), ScmVar' (VarFree "a"));
                  ScmApplicTP' (ScmVar'  (VarFree "b"), [])]))])])))};

  ExprCase {name = "604"; test = Semantic_Analysis.run_semantics;
  input =
        ScmDef (ScmVar "test", ScmLambdaOpt (["x"], "y", ScmApplic (ScmVar "cons", [ScmLambdaSimple (["a"], ScmSeq ([ScmSet (ScmVar "a", ScmVar "f"); ScmOr [ScmVar "a"]])); ScmVar "y"; ScmLambdaSimple ([], ScmOr [ScmSet (ScmVar "y", ScmVar "a"); ScmVar "x"])])));
  expected = ScmDef' (VarFree "test",
  ScmLambdaOpt' (["x"], "y",
      ScmSeq' ([
        ...
        ScmApplicTP' (ScmVar' (VarFree "cons"), [
          ScmLambdaSimple' (["a"],
          ScmSeq' ([
            ScmSet' (VarParam ("a", 0), ScmVar' (VarFree "f"));
            ScmOr' [ScmVar' (VarParam ("a", 0))]
                  ]));
                  ScmBoxGet' (VarParam ("y", 1));
                  ScmLambdaSimple' ([],
                  ScmOr' [ScmBoxSet' (VarBound ("y", 0, 1), ScmVar' (VarFree "a")); ScmVar' (VarBound ("x", 0, 0))])])])))};
];;
