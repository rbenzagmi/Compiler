...
(define fold-left 
	(let ((car car) (cdr cdr) (null? null?))
      (letrec ((loop
        (lambda (f base lst)
          (if (null? lst) base
          	(loop f (f base (car lst)) (cdr lst)))))) loop)))
...
(define reverse-list
  (let ((null? null?) (car car) (cdr cdr) (cons cons))
	(letrec ((reverseLst
		(lambda (lst outLst)
			(if (null? lst) outLst
				(reverseLst (cdr lst) (cons (car lst) outLst))))))
			(lambda (lst) (reverseLst lst '())))))
...
(define fold-right
    (let ((car car) (cdr cdr) (null? null?))
      (letrec ((loop
        (lambda (f base lst)
          (if (null? lst) base
          	(loop f (f (car lst) base) (cdr lst))))))
      (lambda (f base lst)
      	(loop f base (reverse-list lst))))))
